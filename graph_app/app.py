...
class GraphApp(tk.Tk):
    ...
    def _build_widgets(self) -> None:
        ...
        ttk.Label(options_frame, text="Danh sách cạnh (mỗi dòng: u v [w])").grid(
        ...
        ttk.Button(btn_frame, text="Cập nhật đồ thị", command=self._update_graph).pack(
            side=tk.LEFT, padx=5
        )
        ...
        # edge controls
        ttk.Label(crud_frame, text="Cạnh u,v,w").grid(row=1, column=0, padx=5, pady=2)
        ...
        ttk.Label(highlight_frame, text="Highlight đỉnh (ví dụ: A,B)").grid(
        ...
        self.highlight_nodes_entry.bind('<KeyRelease>', lambda e: self._draw_graph())

        ttk.Label(highlight_frame, text="Highlight cạnh (ví dụ: A-B;C-D)").grid(
        ...
        # Auto-update khi thay đổi nội dung
        self.highlight_edges_entry.bind('<KeyRelease>', lambda e: self._draw_graph())
    ...
    def _add_vertex(self) -> None:
        ...
        if not name:
            messagebox.showwarning("Thiếu tên", "Nhập tên đỉnh trước")
            return
    ...
    def _remove_vertex(self) -> None:
        name = self.node_name_entry.get().strip()
        if not name:
            messagebox.showwarning("Thiếu tên", "Nhập tên đỉnh cần xóa")
            return
    ...
    def _draw_graph(self) -> None:
        ...
        for node in nx_graph.nodes():
            if node in highlight_nodes:
                ...
                node_colors.append("white")  # Đỉnh có background màu trắng

        default_edge_color = "#95a5a6"
        edge_colors = []
        for u, v in nx_graph.edges():
            if (u, v) in highlight_edges or (
        ...
        nx.draw_networkx_nodes(
            ...
            node_size=700,
            edgecolors=default_edge_color,  # Border màu đen nhạt giống màu cạnh
            linewidths=2,
        )
    ...
    def _find_clicked_edge(self, click_x, click_y, threshold=0.1):
        """Tìm cạnh gần nhất với điểm click - threshold tăng lên để dễ click hơn"""
        import numpy as np
        nx_graph = self.graph.to_networkx()
        min_dist = float('inf')
        clicked_edge = None
        ...
        for u, v in nx_graph.edges():
            ...
            # Sử dụng công thức khoảng cách điểm đến đường thẳng
            dx = x2 - x1
            dy = y2 - y1
            ...
            # Tham số t cho điểm chiếu trên đường thẳng
    ...
    def _on_mouse_press(self, event):
        ...
        # Kiểm tra click vào node - tăng threshold để dễ click hơn
        node_threshold = 0.2  # Tăng từ 0.15
        clicked_node = None
        ...
        if clicked_node is not None:
            # Click vào node
            if event.dblclick:  # Double click - toggle highlight
                ...
                self._draw_graph()
            else:  # Single click - start dragging
                self.dragging = True
                self.selected_node = clicked_node
                self.canvas.get_tk_widget().config(cursor="hand2")
        else:
            # Không click vào node, kiểm tra click vào edge
            if event.dblclick:  # Chỉ highlight edge khi double click
                clicked_edge = self._find_clicked_edge(click_pos[0], click_pos[1])
                
                if clicked_edge is not None:
                    # Toggle highlight edge
